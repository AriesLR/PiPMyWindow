...
namespace PipMyWindow.Resources.Functions.Services
{
    ...
    {
        private static readonly string currentVersionPipMyWindow = FileVersionInfo.GetVersionInfo(System.Reflection.Assembly.GetExecutingAssembly().Location).FileVersion;
        ...
        public static async Task CheckForUpdatesAsync(string jsonUrl)
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    var latestVersion = updateInfo.latestVersionPipMyWindow;
                    var currentVersion = currentVersionPipMyWindow;
                    ...
                    if (versionComparison < 0)
                    {
                        ...
                        if (userConfirmed)
                        {
                            UrlService.OpenUrlAsync(updateInfo.downloadUrlPipMyWindow);
                        }
                    }
        ...
        {
            public string latestVersionPipMyWindow { get; set; }
            public string downloadUrlPipMyWindow { get; set; }
        }
    }
}